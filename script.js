...
(function() {
    ...
    // THIS SECTION REPLICATES YOUR SHARED MILKY WAY SCRIPT
    function drawMilkyWay() {
        // We use a Linear Gradient to create a diagonal "streak" across the center
        const gradient = ctx.createLinearGradient(0, 0, width, height);
        
        // Exact color stops for a visible space nebula
        gradient.addColorStop(0, "rgba(10, 0, 30, 0)");         // Transparent start
        gradient.addColorStop(0.2, "rgba(75, 0, 130, 0.15)");   // Deep Indigo
        gradient.addColorStop(0.5, "rgba(138, 43, 226, 0.35)"); // VIBRANT STARBASE PURPLE (The Core)
        gradient.addColorStop(0.8, "rgba(75, 0, 130, 0.15)");   // Deep Indigo
        gradient.addColorStop(1, "rgba(10, 0, 30, 0)");         // Transparent end

        ctx.save();
        // "lighter" means the colors add together—the more they overlap, the brighter they get
    ...
    function render(time) {
        ...
        // 1. Draw Nebula first so stars appear "inside" it
        drawMilkyWay();
